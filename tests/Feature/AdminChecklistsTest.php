...
class AdminChecklistsTest extends TestCase
{
    ...
    public function test_manage_checklist_groups()
    {
        ...
        $response = $this->post('admin/checklist-groups', [
            'name' => 'First group'
        ]);
        ...
        $response = $this->get('admin/checklist-groups/'.$group->id.'/edit');
        $response->assertStatus(200);

        $response = $this->put('admin/checklist-groups/'.$group->id, [
        ...
        if (count($menu['user_menu']) > 0){
            $this->assertNull($menu['user_menu']->where('name', 'Updated first group')->get());
        }

        // Test DELETING the checklist group
        $response = $this->delete('admin/checklist-groups/'.$group->id);
        $response->assertRedirect('welcome');
    ...
    public function test_manage_checklists()
    {
        ...
        $checklists_url = 'admin/checklist-groups/' . $checklist_group->id . '/checklists';
    ...
    public function test_manage_tasks()
    {
        ...
        $response->assertRedirect('admin/checklist-groups/' . $checklist_group->id . '/checklists/' . $checklist->id . '/edit');
